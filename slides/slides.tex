...
\subtitle{
    \textbf{\underline{Samuele Giraudo}}\qquad\textbf{St\'ephane~Vialette}\\[1em]
...
\uncover<3->{%
\begin{block}{Example}
    The word $\La\Lb\Lb\La$ is not a square. Its only perfect matching\vspace{-1.25em}
    \begin{equation*}
        \begin{split}
...
it is some cases more convenient to start by defining a \alert{coproduct}
...
\begin{equation*}
    ...
    (x \otimes y)
\end{equation*}
where the $\textcolor{RoyalBlue}{\lambda_{x, y}^z}$ are structure
coefficients of $\QQ$.
\medskip}
...
Let $B := \{\La, \Lb\}^*$ be the set of all binary words and consider
the coproduct
\begin{equation*}
...
\uncover<2->{%
\begin{block}{Example}\vspace{-1em}
\begin{equation*}
    \Delta(\Lb\La\La) =
    \epsilon \otimes \Lb\La\La + \Lb \otimes \La\La
    + 2 (\La \otimes \Lb\La) + 2 (\Lb\La \otimes \La) + \La\La \otimes \Lb
...
\begin{equation*}
    ...
    + \Lb\La\La \otimes \epsilon,
\end{equation*}
we observe that $\textcolor{BrickRed}{\La \otimes \Lb\La}$ has
multiplicity $\textcolor{Orange}{2}$ in
$\textcolor{RoyalBlue}{\Delta(\Lb\La\La)}$.
\bigskip

\uncover<3->{%
By duality, this implies that $\textcolor{RoyalBlue}{\Lb\La\La}$ has
multiplicity $\textcolor{Orange}{2}$ in
$\textcolor{BrickRed}{\La \shuffle \Lb\La}$. Indeed,
\begin{equation*}
