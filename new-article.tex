...
\newcommand{\Crossing}{%
\begin{tikzpicture}[scale=0.09,inner sep=.8pt,node distance=.25cm,>=latex']%,>=stealth',->]
...
\begin{tikzpicture}[scale=#1,inner sep=.8pt,node distance=.45cm,>=latex']%,>=stealth',->]
\draw node [] (U1)               {#2};
...
\newcommand{\Inclusion}{%
\begin{tikzpicture}[scale=0.09,inner sep=.8pt,node distance=.25cm,>=latex']
\draw node [draw,circle,fill=black] (U1)               {};
\draw node [draw,circle,fill=black] [right of=U1] (U2) {};
\draw node [draw,circle,fill=black] [right of=U2] (U3) {};
\draw node [draw,circle,fill=black] [right of=U3] (U4) {};
...
\begin{abstract}
    ...
    permutations is \NPC. This property is proved by establishing a
...
Let us now provide some definitions about graphs and matching, used in
...
graph. In case the set of vertices is equipped with a total order, a
...
\begin{proof}[of Lemma~\ref{lemma:matching}]
    ...
    Then, by definition, there exist two sets
    $I^1 = (i^1_1 < i^1_2 < \dots < i^1_n)$
    ...
    Let $G_\pi$ be the linear clique associated to $\pi$ with vertices
    $\VS{G_\pi} = \{i^1_j : j \in [n]\} \cup \{i^2_j : j \in [n]\}$.
    Define $\vv{\mathcal{M}} = \{(i^1_j, i^2_j) : j \in [n]\}$.
    It is easily seen that $\vv{\mathcal{M}}$ is an oriented perfect
    matching since $I^1 \cap I^2 = \emptyset$, and
    $I^1 \cup I^2 = \{1, 2,  \dots 2n\}$.
    We first show that it avoids
    \InclusionLL,~\InclusionLR,~\InclusionRL and \InclusionRR.
    Indeed, suppose, aiming at a contradiction, that such a bad configuration exists
    for, say,  arcs $(i^1_j, i^2_j)$ and $(i^1_k, i^2_k)$ of $\vv{\mathcal{M}}$.
    Assuming $i^1_j < i^1_k$ we are left with the following $4$ configurations:
    \begin{center}
      \begin{tabular}{cccc}
      ...
      \end{tabular}
    \end{center}
    Then it follows that $i^2_j > i^2_k$.
    This is a contradiction since
    $i^1_j < i^1_k \Rightarrow j < k \Rightarrow i^2_j < i^2_k$.
    We now turn to proving that $\vv{\mathcal{M}}$ also avoids
    \CrossingLR and \CrossingRL.
    Indeed, suppose, aiming at a contradiction, that such a bad configuration exists
    for, say,  arcs $(i^1_j, i^2_j)$ and $(i^1_k, i^2_k)$ of $\vv{\mathcal{M}}$.
    Assuming $i^1_j < i^1_k$ we are left with the following $2$ configurations:
    \begin{center}
      \begin{tabular}{cccc}
    ...
    $i^1_j < i^1_k \Rightarrow j < k \Rightarrow i^2_j < i^2_k$.
    Finally, for any two distinct arcs
    $(i^1_j, i^2_j)$ and $(i^1_k, i^2_k)$ of $\vv{\mathcal{M}}$,
    ...
    Let $I^1 = (i^1_1 < i^1_2 < \dots < i^1_n)$
    (resp. $I^2 = (i^2_1 < i^2_2 < \dots < i^2_n)$)
    be the ordered sequence of the origins (resp. sinks) of the arcs of
    the oriented perfect matching $\vv{\mathcal{M}}$.
    We first show that, for every $1 \leq j \leq n$,
    $(i^1_j, i^2_j)$ is an arc of $\vv{\mathcal{M}}$.
    ...
    \begin{itemize}
      ...
      \begin{center}
        \begin{tabular}{cccc}
          ...
          &\qquad
          \LabeledInclusionRR{.3}{$i^1_p$}{$i^2_r$}{$i^1_q$}{$i^1_n$}
          &\qquad
    ...
    from $\pi$ induced by $I^1$ and $I^2$, respectively.
    Clearly $p^1$ and $p^2$ are disjoint in $\pi$
    (since $\vv{\mathcal{M}}$ is a matching)
    and cover $\pi$ (since $\vv{\mathcal{M}}$ is perfect).
    We claim that $p^1$ and $p^2$ are order-isomorphic.
    Indeed, this is a direct consequence of th fact that,
    for any two distinct edges $(a, b)$ and $(a', b')$ in
    $\vv{\mathcal{M}}$ we have $\pi(a) < \pi(a')$ if and only if
    $\pi(b) < \pi(b')$.
    \qed
\end{proof}
...
\begin{proof}[of Lemma~\ref{lemma:at most one edge monotone}]
  ...
  Therefore, since $\vv{\mathcal{M}}$ avoids
  \InclusionLL,~\InclusionLR,~\InclusionRL,~\InclusionRR,~\CrossingLR
  and \CrossingRL,
