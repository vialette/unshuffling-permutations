...
the shuffle product of words leads to the following algorithmic ones:
\begin{enumerate}
...
from an algorithmic point of view by focusing on {\em square}
...
Let us now provide some definitions about graphs and matchings that are used in
...
\begin{figure}[ht!]
  ...
  \caption{\label{fig:example containment-free matching}%
    The linear graph $K_\pi$ associated to the permutation
...
\begin{proof}[of Claim~\ref{claim:binary_to_permutation_avoiding}]
    ...
    occurrence of $21$ (resp. $2$, $1$) in $w$. All these three cases
...
\begin{proof}[of Lemma~\ref{lemma:matching}]
    ...
    $|\sigma| = n$). Let $K_\pi$ be the linear clique associated to $\pi$
    with vertices
