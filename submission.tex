...
Moreover, $G$ is {\em perfect} if all vertices of $G$ belong to at least 
one arc. For any word $u = u_1 u_2 \dots u_n$, an 
{\em oriented perfect matching on $u$} is an oriented perfect matching 
$\mathcal{M}$ on the set of vertices $\{u_1, u_2, \dots, u_n\}$. In the 
sequel, we shall consider a natural notion of pattern avoidance in 
oriented perfect matchings on words. For instance, an oriented perfect
matching $\mathcal{M}$ of a word $u$ {\em admits an occurrence} of the 
pattern $\CrossingRL$ if there are four positions $i < j < k < \ell$ in 
$u$ such that $(u_k, u_i)$ and $(u_j, u_\ell)$ are arcs of $\mathcal{M}$. 
When $\mathcal{M}$ does not admit any occurrence of a pattern 
$\mathcal{P}$, we say that $\mathcal{M}$ {\em avoids}~$\mathcal{P}$.
...
\begin{definition}[Property $\mathbf{P_1}$]
  ...
  on $\pi$ is said to have property $\mathbf{P_1}$ if it avoids the 
  patterns \InclusionLL, \InclusionLR, \InclusionRL, 
...
\begin{definition}[Property $\mathbf{P_2}$]
  ...
  distinct arcs $(a, a')$ and $(b, b')$ in $\mathcal{M}$,
