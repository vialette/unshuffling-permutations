...
from each word. In addition to purely combinatorial questions, the
shuffle product of words leads to the following computational questions:
\begin{enumerate}
    \item \label{item:problem_1}
    Given three words $u$, $v$, and $w$, decide if $u$ is a shuffle of
    $v$ and $w$.
    \item \label{item:problem_2}
    Given a word $u$, decide if there exist two words $v$ and $w$ such
    that $u$ is a shuffle of $v$ and $w$.
\end{enumerate}
Problem~\ref{item:problem_1} is in $\Pclass$ (it is a classical
textbook exercise to design an efficient dynamic programming algorithm
that determines whether $u$ is a shuffle of $v$ and $w$), while
Problem~\ref{item:problem_2} is $\NPclass$-complete as shown
in~\cite{Rizzi:Vialette:CSR:2013}. Analogous questions can be stated to
decide if a word $u$ is a \emph{square} (w.r.t. the shuffle), that is
a word $u$ with the property that there exists a word $v$ such that
$u$ is a shuffle of $v$ with itself.
\smallskip

This paper is intended to study a natural generalization $\SHUFFLE$
of the shuffle
of permutations. Roughly speaking, given three permutations $\pi$, $\mu$,
and $\nu$, $\pi$ is said to be a {\em shuffle} of $\mu$ and $\nu$ if $\pi$
(viewed as a word) is a shuffle of two words that are order-isomorphic
to $\mu$ and $\nu$.
%For
...
Our intention in this paper is to study this shuffle product of
permutations both from a combinatorial and from a computational point of
view by focusing on {\em square} permutations, that are permutations
$\pi$ being in the shuffle of a permutation $\mu$ with itself. The
shuffle product defined and considered in this paper is different from
other existing and well-known shuffle products on permutations. For
instance, in~\cite{DHT:IJAC:2002}, the authors define the
{\em shifted shuffle product}. For this product, $\pi$ is a shuffle of
$\mu$ and $\nu$ if $\pi$ is in the shuffle, as words, of $\mu$ and the
word obtained by incrementing all the letters of $\nu$ by the size of $\mu$.
It is a simple exercise to prove that, given three permutations $\pi$,
$\mu$, and $\nu$, deciding if $\pi$ is a in the shifted shuffle of
$\mu$ and $\nu$ is in $\Pclass$.
\smallskip

Let us now  briefly review the key results for words that arise in our context.
...
Given three words $u$, $v_1$, and $v_2$, it can be tested in
$O\left(|u|^2 / \log(|u|)\right)$ time whether or not
$u$ is in the shuffle $v_1 \shuffle v_2$ of $v_1$ and
$v_2$~\cite{Leeuwen:Nivat:IPL:1982}.
To the best of our knowledge, the first $O(|u|^2)$ time algorithm
appeared in~\cite{Mansfield:DAM:1983}. This algorithm can easily be
...
$u_1$, $u_2$, \dots, $u_k$, the shuffle $\shuffle_{i=1}^{k} u_i$ can be
computed in
\begin{equation}
    O\left(\binom{|u_1|+|u_2|+\ldots+|u_k|}{|u_1|,|u_2|,\ldots,|u_k|}\right)
\end{equation}
time. Given words $u$, $v_1$, $v_2$, \dots, $v_n$, it is however \NPC
to decide whether or not $u$ is in $\shuffle_{i=1}^{k} v_i$.
\cite{Mansfield:DAM:1983,Warmuth:Haussler:JCSS:1984}.
This remains true even if the ground alphabet has size $3$~\cite{Warmuth:Haussler:JCSS:1984}.
...
Again, this remains true even if the ground alphabet has size $3$.
...
Section~\ref{section:Shuffle product on permutations} we provide a precise
definition of $\SHUFFLE$. This definition passes
through the preliminary definition of an operator $\Delta$ to unshuffle
permutations. This operator is in fact a coproduct, endowing the space
of all permutations with a coalgebra structure. By duality, the
unshuffling operator $\Delta$ leads to the definition of our shuffle operation on
permutations. This approach has many advantages. First, some combinatorial
properties of $\SHUFFLE$ depend on properties of $\Delta$
and are more easy to prove on the coproduct
side. Second, this way of doing allows to obtain a clear description of
the multiplicities of the elements appearing in the shuffle of two
permutations, which are worthy of interest.
Section~\ref{section:Binary square words and permutations}
is devoted to show that the problems related to the shuffle of words has
links with the shuffle of permutations. In particular, we show that
binary words that are square are in one-to-one bijection with square
permutations (Proposition~\ref{prop:bijection_binary_to_permutations_squares}).
Next, Section~\ref{section:Algebraic issues} presents some algebraic
and combinatorial properties of~$\SHUFFLE$. In particular, we show that
$\SHUFFLE$ is associative and commutative
(Proposition~\ref{prop:shuffle_associative_commutative}), and that
if a permutation if a square, its mirror, complement, and inverse also
are square (Proposition~\ref{prop:square_stability}). Finally,
Section~\ref{section:Algorithmic issues} presents
the most import result of this paper: the fact that deciding if
a permutation is a square is $\NPclass$-complete
...
fact that we shall denote by $\pi(i)$ (instead of $\pi_i$) the
$i$-th letter of any permutation $\pi$.
...
\begin{proof}[of Lemma~\ref{lemma:matching}]
  ...
  %Let $\sigma$ be a permutation such that $\pi$ is the union of two
  %patterns that are order-isomorphic to $\sigma$.
  Let $2n = |\pi|$ (and hence $|\sigma| = n$).
  %Fix the occurrences in $\pi$ of the two disjoint patterns that
  %are order-isomorphic to $\sigma$ and
  %write
  %$I^1 = \{i^1_1, i^1_2, \ldots, i^1_n\}$,
  %$i^1_1 < i^1_2 < \ldots < i^1_n$, for the positions in $\pi$
  %of the first pattern order-ismorphic to $\sigma$ and
  %$I^2 = \{i^2_1, i^2_2, \ldots, i^2_n\}$,
  %$i^2_1 < i^2_2 < \ldots < i^2_n$, for the positions in $\pi$
  %of the second pattern order-ismorphic to $\sigma$.
...
\begin{proof}
  ...
  Given two permutations $\pi$ and $\sigma$, decide whether $\sigma$
