...
\begin{proof}
    ...
    $P_1^\sim := \{|\pi| - i  + 1 : i \in P_1\}$,
    $P_2^\sim := \{|\pi| - i  + 1 : i \in P_2\}$,
    $u' := \pi^\sim_{|P_1^\sim}$, and $v' := \pi^\sim_{|P_2^\sim}$.
    Now, by construction, ${u'}^\sim = u$ and ${v'}^\sim = v$. Then,
    by~\eqref{equ:square_stability_same_std},
    $\STD(u') = \rho^\sim = \STD(v')$, so that $\rho^\sim$ is a square
    root of $\pi$.
    \smallskip
