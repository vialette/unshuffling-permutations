...
We write $\nearrow_{n}$ (resp. $\searrow_{n}$)
for the permutation $1 2 \ldots n$ (resp. $n (n-1) \ldots 1$).
we write seqeunce of integers $s = s_1 s_2 \ldots s_n$, we
we let $s\;[k]$ stand for the lifted sequence
...
\begin{proof}[of Lemma~\ref{lemma:matching}]
  ...
  Let $G_\pi$ the linear clique associated to $\pi$ with
  vertices $\VS{G_\pi} =
  ...
  Indeed, if it were not the case then there would exist two edges
  $e = \{i^1_j, i^2_j\}$ and
  $e' = \{i^1_k, i^2_k\}$, $j < k$,
  ...
  both $\{i^1_j, i^2_j\}$ and $\{i^1_k, i^2_k\}$.
  This is a contradiction since
  for any two disctinct edges
  $(a, b)$ and $(a', b')$ in $\mathcal{M}$
  we have $\pi(a) < \pi(a')$ if and only if $\pi(b) < \pi(b')$.
...
strings of length $2n$ are squares; Problem~4
