...
The {\em shuffle product} $\shuffle$ is a very famous operation on words
first defined by Eilenberg and MacLane~\cite{Eilenberg:MacLane:1953}.
Given three words $u$, $v$, and $w$, $u$ is said to be a \emph{shuffle}
of $v$ and $w$ if it can be formed by interleaving the letters from $v$
and $w$ in a way that maintains the left-to-right ordering of the
...
\begin{enumerate}
    ...
    Given a word $u$, decide if there exist two words $v_1$ and $v_2$
    such that $u$ is a shuffle of $v_1$ and $v_2$.
\end{enumerate}
...
Let us now focus on Problem~\ref{item:problem_4}. It is shown
...
is a shuffle of $v$ with itself. The hardness of this last particular
problem implies that Problem~\ref{item:problem_4} is \NPC.
\smallskip
...
given three permutations $\pi$, $\mu$, and $\nu$, $\pi$ is said to be a
{\em shuffle} of $\mu$ and $\nu$ if $\pi$ (viewed as a word) is a
shuffle of two words that are order-isomorphic to $\mu$ and $\nu$.
Our intention in this paper is to study this shuffle product of
permutations $\SHUFFLE$ both from a combinatorial and from a
computational point of view by focusing on {\em square} permutations,
that are permutations $\pi$ being in the shuffle of a permutation $\mu$
with itself. The shuffle product defined and considered in this paper
is different from other existing and well-known shuffle products on
permutations. For instance, in~\cite{DHT:IJAC:2002}, the authors define
the {\em shifted shuffle product}. For this product, $\pi$ is a shuffle
of $\mu$ and $\nu$ if $\pi$ is in the shuffle, as words, of $\mu$ and
the word obtained by incrementing all the letters of $\nu$ by the size
of $\mu$. It is a simple exercise to prove that, given three
permutations $\pi$, $\mu$, and $\nu$, deciding if $\pi$ is a in the
shifted shuffle of $\mu$ and $\nu$ is in~\Pclass.
\smallskip
...
preliminary definition of an operator $\Delta$ to unshuffle permutations.
This operator is in fact a coproduct, endowing the space of all
permutations with a coalgebra structure. By duality, the unshuffling
...
of interest. Section~\ref{section:Binary square words and permutations}
...
combinatorial properties of~$\SHUFFLE$. In particular, we show that
...
the most import result of this paper: the fact that deciding if a
permutation is a square is \NPC (Proposition~\ref{proposition:hardness}).
