...
shuffle product of words leads to the following purely computational
questions:
\begin{enumerate}
...
\end{enumerate}
Problem~\ref{item:problem_1} is in $\Pclass$ (it is a classical textbook
exercise to design an efficient dynamic programming algorithm that
determines whether $u$ is a shuffle of $v$ and $w$), while
Problem~\ref{item:problem_2} is in $\NPclass$ as shown
in~\cite{Rizzi:Vialette:CSR:2013}. Analogous questions can be stated to
decide if a word $u$ is a \emph{square} (w.r.t. the shuffle). A square
is a word $u$ with the property that there exists a word $v$ such that
$u$ is a shuffle of $v$ with itself.
\smallskip

This paper is intended to study a natural generalization of the shuffle
to permutations. Roughly speaking, given three permutations $\pi$, $\mu$,
and $\nu$, $\pi$ is said to be a shuffle of $\mu$ and $\nu$ if $\pi$
...
Our intention in this paper is to to study this shuffle product of
...
$u$ is in $v_1 \shuffle v_2$~\cite{Leeuwen:Nivat:IPL:1982}.
