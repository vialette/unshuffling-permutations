...
a coproduct denoted by $\Delta$. By duality, $\Delta$ implies the
definition of $\SHUFFLE$. The reason why we need to pass by the
definition of $\Delta$ to define $\SHUFFLE$ is justified by the fact
that a lot of properties of $\SHUFFLE$ depend of properties of $\Delta$
and that this strategy allows to write concise and clear proofs of them.
We invite the reader unfamiliar with the concepts of coproduct and
duality to consult~\cite{Joni:Rota:1979} or~\cite{Grinberg:Reiner:2014}.
\medskip
...
such that $P_1 \sqcup P_2 = [|\pi|]$, the subword $\pi_{|P_1}$ is
order isomorphic to $\sigma$ and the subword $\pi_{|P_2}$ is order
isomorphic to $\nu$.
\medskip
...
are two sets $P_1$ and $P_2$ of disjoints indexes of letters of $\pi$
such that $P_1 \sqcup P_2 = [|\pi|]$, and the subword $\pi_{|P_1}$ and
$\pi_{|P_2}$ are order isomorphic.
\medskip
...
of permutation defined in
Section~\ref{section:Shuffle product on permutations} imply combinatorial
